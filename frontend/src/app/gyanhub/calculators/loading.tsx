    ...
    <div className="min-h-screen bg-[#050505] text-slate-100 font-sans">
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-white/[0.02] rounded-[1.5rem] p-4 border border-white/5 h-20 animate-pulse" />
          ))}
        </div>
        <div className="bg-white/[0.02] rounded-[2.5rem] p-8 border border-white/5 h-64 animate-pulse" />
      </div>
    </div>
  );
}
