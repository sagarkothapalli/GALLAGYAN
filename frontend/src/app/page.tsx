...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
        if (stock) {
            refreshCurrentStock(stock.symbol.replace('.NS', '').replace('.BO', ''));
        } else {
            fetchMarketNews();
        }
        checkHealth();
  ...
  const calculatePortfolioSummary = () => {
    ...
    return { totalInv, totalVal, pnl: totalVal - totalInv, pnlPercent: totalInv > 0 ? ((totalVal - totalInv) / totalInv) * 100 : 0 };
  };
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-6 items-center">
          ...
          <div className="flex-1 w-full relative" ref={searchRef}>
            <form onSubmit={(e) => { e.preventDefault(); if (ticker) fetchStock(ticker); }} className="relative">
              <div className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg></div>
              <input type="text" value={ticker} onFocus={() => setShowSuggestions(true)} onChange={(e) => setTicker(e.target.value.toUpperCase())} placeholder="Search stocks, indices..." className="w-full bg-slate-100/80 border-none rounded-2xl py-3.5 pl-12 pr-6 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/10 transition-all placeholder:text-slate-400 font-medium" />
              <div className="absolute right-3 top-1/2 -translate-y-1/2"><button type="submit" disabled={loading} className="bg-slate-900 hover:bg-black text-white px-4 py-1.5 rounded-xl text-xs font-bold transition-all disabled:opacity-50">{loading ? '...' : 'Search'}</button></div>
            </form>
            {showSuggestions && suggestions.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-3 bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-2xl z-[100] animate-in fade-in slide-in-from-top-2 duration-200 ring-1 ring-black/5">
                {suggestions.map((s) => (
                  <button key={s.symbol} onClick={() => { setTicker(s.symbol); fetchStock(s.symbol); }} className="w-full flex items-center justify-between px-6 py-4 hover:bg-slate-50 border-b border-slate-50 last:border-0 text-left transition-colors group">
                    <div>
              ...
              <section className="bg-white rounded-[2rem] p-8 md:p-10 border border-slate-200/60 shadow-sm relative">
                <div className="flex flex-col md:flex-row justify-between items-start gap-8">
                  ...
                  <div className="text-left md:text-right">
                    ...
                    <div className={`text-xl md:text-2xl font-bold mt-2 flex items-center md:justify-end gap-3 ${stock.change >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}><span className="flex items-center gap-1.5">{stock.change >= 0 ? '▲' : '▼'} {Math.abs(stock.change).toFixed(2)}</span><span className="text-sm bg-slate-50 px-3 py-1 rounded-xl border border-slate-100/80 font-bold">{stock.percent_change.toFixed(2)}%</span></div>
                  </div>
                </div>
              </section>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex p-1.5 bg-white border border-slate-200/60 rounded-2xl w-full md:w-max shadow-sm overflow-hidden">
                        {(['chart', 'financials', 'news', 'portfolio'] as const).map((tab) => (
                            <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 md:w-32 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-slate-900 text-white' : 'text-slate-400 hover:text-slate-600'}`}>{tab}</button>
                        ))}
                    </div>
                    {activeTab === 'chart' && (
                        <div className="flex p-1 bg-slate-100 rounded-xl overflow-hidden shadow-inner">
                            {PERIODS.map(p => (
                                <button key={p.label} onClick={() => setChartPeriod(p)} className={`px-4 py-1.5 text-[10px] font-black transition-all ${chartPeriod.label === p.label ? 'bg-white text-blue-600 shadow-sm rounded-lg' : 'text-slate-400 hover:text-slate-600'}`}>{p.label}</button>
                            ))}
                        </div>
                    )}
                </div>
                  ...
                  {activeTab === 'news' && (
                    <div className="bg-white rounded-[2rem] p-6 md:p-10 border border-slate-200/60 shadow-sm">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {news.length > 0 ? news.map((item, idx) => (<NewsCard key={idx} item={item} />)) : <p className="col-span-full text-center text-slate-400 py-10 text-sm font-bold">No recent news available.</p>}
                      </div>
                    </div>
                  )}
                  {activeTab === 'portfolio' && (
                    <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="bg-white p-6 rounded-3xl border border-slate-200/60 shadow-sm">
                                ...
                                <p className="text-2xl font-black text-slate-900">₹{portfolioSummary.totalVal.toLocaleString('en-IN')}</p>
                            </div>
                            <div className="bg-white p-6 rounded-3xl border border-slate-200/60 shadow-sm">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Profit / Loss</p>
                                <p className={`text-2xl font-black ${portfolioSummary.pnl >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{portfolioSummary.pnl >= 0 ? '+' : ''}₹{portfolioSummary.pnl.toLocaleString('en-IN')}</p>
                            </div>
                            <div className="bg-emerald-600 p-6 rounded-3xl shadow-lg shadow-emerald-500/20 text-white">
                                <p className="text-[10px] font-black text-white/70 uppercase tracking-widest mb-1">Return %</p>
                                <p className="text-2xl font-black">{portfolioSummary.pnlPercent.toFixed(2)}%</p>
                            </div>
                        </div>
                        <div className="bg-white rounded-[2rem] p-8 border border-slate-200/60 shadow-sm space-y-8">
                            <div className="flex flex-col md:flex-row justify-between items-center gap-6 pb-8 border-b border-slate-100">
                                <div><h3 className="text-xl font-bold text-slate-900">Add to Holdings</h3><p className="text-xs text-slate-400 font-medium">Record buy price for {stock?.name}</p></div>
                                <div className="flex gap-3">
                                    <input type="number" placeholder="Buy Price" value={portfolioInput.buyPrice} onChange={e => setPortfolioInput(prev => ({ ...prev, buyPrice: e.target.value }))} className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-sm w-32 focus:outline-none focus:ring-2 focus:ring-blue-500/10" />
                                    <input type="number" placeholder="Qty" value={portfolioInput.quantity} onChange={e => setPortfolioInput(prev => ({ ...prev, quantity: e.target.value }))} className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-sm w-24 focus:outline-none focus:ring-2 focus:ring-blue-500/10" />
                            ...
                            <div className="space-y-4">
                                {portfolio.map(item => {
                                    ...
                                    return (
                                        <div key={item.symbol} className="flex justify-between items-center bg-slate-50/50 p-6 rounded-3xl border border-slate-100 transition-all hover:bg-white hover:shadow-md">
                                            <div className="flex items-center gap-4"><div className="w-10 h-10 bg-white rounded-xl border border-slate-200 flex items-center justify-center font-bold text-xs text-slate-400">{item.symbol[0]}</div><div><p className="text-sm font-bold text-slate-900">{item.symbol}</p><p className="text-[10px] text-slate-400 font-bold uppercase">{item.quantity} Shares @ ₹{item.buyPrice}</p></div></div>
                                            <div className="text-right flex items-center gap-6"><div><p className={`text-sm font-black ${pnl >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{pnl >= 0 ? '+' : ''}₹{pnl.toLocaleString('en-IN')}</p></div><button onClick={() => removeFromPortfolio(item.symbol)} className="text-slate-300 hover:text-rose-500">✕</button></div>
                                        </div>
                                    );
                                })}
                            </div>
