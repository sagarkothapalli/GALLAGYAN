...
export default function Home() {
  ...
  const fetchStock = async (symbol: string) => {
    setLoading(true); setError(''); setShowSuggestions(false); setActiveTab('chart'); setHistory([]); setNews([]); setFundamentals([]); setActions(null);
    try {
      ...
      const [newsRes, fundRes, actionRes] = await Promise.all([fetch(`${baseUrl}/api/stock/${cleanSymbol}/news`), fetch(`${baseUrl}/api/stock/${cleanSymbol}/fundamentals`), fetch(`${baseUrl}/api/stock/${cleanSymbol}/actions`)]);
      if (newsRes.ok) setNews(await newsRes.json()); if (fundRes.ok) setFundamentals(await fundRes.json()); if (actionRes.ok) setActions(await actionRes.json());
