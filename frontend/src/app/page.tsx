...
export default function Home() {
  ...
  const fetchStock = async (symbol: string) => {
    setLoading(true); setError(''); setShowSuggestions(false); setActiveTab('chart'); setHistory([]); setNews([]); setFundamentals([]); setActions(null); setPeersData(null); setProfile(null); setComparisonData(null);
    try {
      ...
      const [newsRes, fundRes, actionRes, peerRes, profileRes] = await Promise.all([fetch(`${baseUrl}/api/stock/${cleanSymbol}/news`), fetch(`${baseUrl}/api/stock/${cleanSymbol}/fundamentals`), fetch(`${baseUrl}/api/stock/${cleanSymbol}/actions`), fetch(`${baseUrl}/api/stock/${cleanSymbol}/peers`), fetch(`${baseUrl}/api/stock/${cleanSymbol}/profile`)]);
      if (newsRes.ok) setNews(await newsRes.json()); if (fundRes.ok) setFundamentals(await fundRes.json()); if (actionRes.ok) setActions(await actionRes.json()); if (peerRes.ok) setPeersData(await peerRes.json()); if (profileRes.ok) setProfile(await profileRes.json());
              ...
              <div className="space-y-6">
                ...
                <div className="animate-in fade-in duration-500">
                  {activeTab === 'chart' && (<div className="bg-white dark:bg-[#0a0a0a] rounded-[2rem] p-6 md:p-8 border border-slate-200/60 dark:border-white/5 shadow-sm relative min-h-[400px]">
                        <div className="flex flex-col md:flex-row justify-between gap-4 mb-6 pb-6 border-b border-slate-50 dark:border-white/5">
                            ...
                            <div className="relative">
                                <input type="text" value={comparisonTicker} onChange={e => setComparisonTicker(e.target.value.toUpperCase())} onKeyDown={e => { if (e.key === 'Enter' && comparisonTicker) fetchComparison(comparisonTicker, chartPeriod); }} placeholder="Compare with symbol..." className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 rounded-xl px-4 py-1.5 text-[10px] font-bold focus:outline-none w-48" />
                                {comparisonData && <button onClick={() => setComparisonData(null)} className="absolute right-2 top-1/2 -translate-y-1/2 text-rose-500 text-xs font-black px-2 bg-rose-50 dark:bg-rose-900/20 rounded-lg">CLEAR</button>}
                            </div>
                        </div>
                  ...
                  {activeTab === 'financials' && (<div className="space-y-8 animate-in fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="bg-white dark:bg-[#0a0a0a] rounded-[2rem] p-8 border border-slate-200/60 dark:border-white/5 shadow-sm space-y-6"><h3 className="text-lg font-bold text-slate-900 dark:text-white">Price Alerts</h3><div className="flex gap-3"><select value={alertInput.type} onChange={e => setAlertInput(prev => ({ ...prev, type: e.target.value as any }))} className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 rounded-xl px-3 py-2 text-xs font-bold focus:outline-none dark:text-white"><option value="ABOVE">Above</option><option value="BELOW">Below</option></select><input type="number" placeholder="Price" value={alertInput.price} onChange={e => setAlertInput(prev => ({ ...prev, price: e.target.value }))} className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 rounded-xl px-4 py-2 text-sm flex-1 focus:outline-none dark:text-white" /><button onClick={addAlert} className="bg-blue-600 text-white px-6 py-2 rounded-xl text-xs font-bold hover:bg-blue-700 shadow-lg shadow-blue-500/20">Set</button></div></div>{profile?.summary && (<div className="bg-white dark:bg-[#0a0a0a] rounded-[2rem] p-8 border border-slate-200/60 dark:border-white/5 shadow-sm"><h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Business Summary</h3><div className="flex gap-4 mb-6"><span className="bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest">{profile.sector}</span><span className="bg-slate-50 dark:bg-white/5 text-slate-500 dark:text-slate-400 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest">{profile.industry}</span></div><p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed line-clamp-6 hover:line-clamp-none transition-all cursor-pointer">{profile.summary}</p></div>)}</div>
