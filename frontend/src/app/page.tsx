...
export default function Home() {
  ...
  const fetchStock = async (symbol: string) => {
    ...
    try {
      ...
      const [newsRes, fundRes, actionRes, peerRes, profileRes, calRes] = await Promise.all([
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {calendar && (
                            <div className="bg-white dark:bg-[#0a0a0a] rounded-[2rem] p-8 border border-slate-200/60 dark:border-white/5 shadow-sm space-y-6">
                                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Corporate Calendar</h3>
                                <div className="space-y-4">
                                    <div className="flex justify-between py-2 border-b border-slate-50 dark:border-white/5"><span className="text-xs text-slate-400 font-bold uppercase">Next Earnings</span><span className="text-sm font-bold text-blue-600">{calendar.earnings_date ? new Date(calendar.earnings_date).toLocaleDateString() : 'TBD'}</span></div>
                                    <div className="flex justify-between py-2 border-b border-slate-50 dark:border-white/5"><span className="text-xs text-slate-400 font-bold uppercase">Est. Revenue (Avg)</span><span className="text-sm font-bold">{calendar.revenue_avg ? `₹${(calendar.revenue_avg / 10000000).toLocaleString('en-IN')} Cr` : '-'}</span></div>
                                    <div className="flex justify-between py-2 border-b border-slate-50 dark:border-white/5"><span className="text-xs text-slate-400 font-bold uppercase">Est. EPS (Avg)</span><span className="text-sm font-bold">{calendar.eps_avg ? `₹${calendar.eps_avg.toFixed(2)}` : '-'}</span></div>
                                </div>
                            </div>
                        )}
