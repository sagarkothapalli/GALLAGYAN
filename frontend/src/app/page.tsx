        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          {stock ? (
            <div className="space-y-8 animate-in slide-in-from-bottom-8 duration-700">
              <section className="bg-white rounded-[2rem] p-8 md:p-10 border border-slate-200/60 shadow-sm relative"><div className="flex flex-col md:flex-row justify-between items-start gap-8"><div className="space-y-4"><div className="flex items-center gap-4"><h2 className="text-4xl md:text-5xl font-extrabold tracking-tight text-slate-900">{stock.name}</h2><div className="flex items-center gap-2"><button onClick={(e) => toggleWatchlist(e, stock.symbol)} className={`transition-all hover:scale-110 p-2.5 rounded-2xl ${watchlist.includes(stock.symbol.replace('.NS', '').replace('.BO', '')) ? 'bg-yellow-50 text-yellow-400' : 'bg-slate-100 text-slate-300'}`}><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg></button><button onClick={() => { navigator.clipboard.writeText(window.location.href); alert('Analysis link copied!'); }} className="p-2.5 bg-slate-50 text-slate-300 rounded-2xl hover:text-blue-500 transition-colors"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/></svg></button></div></div><div className="flex items-center gap-3"><span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-lg text-[10px] font-bold tracking-widest border border-slate-200/50">NSE India</span><span className="text-slate-400 font-mono font-medium text-sm">{stock.symbol}</span>{stock.is_volume_spike && <span className="bg-amber-50 text-amber-600 px-3 py-1 rounded-lg text-[9px] font-black uppercase border border-amber-100 animate-pulse">Volume Spike</span>}</div></div><div className="text-left md:text-right"><div className="text-5xl md:text-7xl font-bold tabular-nums text-slate-900">₹{stock.price.toLocaleString('en-IN')}</div><div className={`text-xl md:text-2xl font-bold mt-2 flex items-center md:justify-end gap-3 ${stock.change >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}><span className="flex items-center gap-1.5">{stock.change >= 0 ? '▲' : '▼'} {Math.abs(stock.change).toFixed(2)}</span><span className="text-sm bg-slate-50 px-3 py-1 rounded-xl border border-slate-100/80 font-bold">{stock.percent_change.toFixed(2)}%</span></div></div></div></section>
              <div className="space-y-6">
                ...
                <div className="animate-in fade-in duration-500">
                  ...
                  {activeTab === 'financials' && (<div className="space-y-8 animate-in fade-in">
                    ...
                    {profile?.summary && (<div className="bg-white rounded-[2rem] p-8 border border-slate-200/60 shadow-sm"><h3 className="text-lg font-bold text-slate-900 mb-4">Business Summary</h3><div className="flex gap-4 mb-6"><span className="bg-blue-50 text-blue-600 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest">{profile.sector}</span><span className="bg-slate-50 text-slate-500 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest">{profile.industry}</span></div><p className="text-sm text-slate-600 leading-relaxed line-clamp-6 hover:line-clamp-none transition-all cursor-pointer">{profile.summary}</p></div>)}
            ...
            <div className="space-y-12 animate-in fade-in duration-1000">
              ...
              <div className="bg-white p-8 rounded-[2rem] border border-slate-200/60 shadow-sm space-y-6">
                <div className="flex items-center justify-between"><h3 className="text-xl font-bold text-slate-900">Market Sentiment</h3>{marketNews?.sentiment && <span className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest ${marketNews.sentiment === 'Bullish' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-rose-50 text-rose-600 border border-rose-100'}`}>{marketNews.sentiment} Pulse</span>}</div>
                <p className="text-sm text-slate-500 font-medium leading-relaxed">Aggregated news sentiment indicates a <strong className={marketNews?.sentiment === 'Bullish' ? 'text-emerald-600' : 'text-rose-600'}>{marketNews?.sentiment?.toLowerCase() || 'neutral'}</strong> bias in the Indian markets today.</p>
              </div>
              ...
              <div className="space-y-6"><div className="flex items-center justify-between px-2"><h3 className="text-xl font-bold text-slate-900">Market Pulse</h3><span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-3 py-1 rounded-full border border-blue-100">LIVE FEED</span></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{(marketNews?.articles || []).map((item: any, idx: number) => (<NewsCard key={idx} item={item} />))}</div></div>
            </div>
          )}
        </div>
...
function NewsCard({ item }: { item: NewsItem }) {
  ...
  const sentimentColor = item.sentiment === 'Bullish' ? 'text-emerald-600 bg-emerald-50' : item.sentiment === 'Bearish' ? 'text-rose-600 bg-rose-50' : 'text-slate-400 bg-slate-50';
  return (
    <a href={item.link} target="_blank" rel="noopener noreferrer" className={`flex flex-col gap-4 p-6 rounded-3xl transition-all hover:scale-[1.02] shadow-sm hover:shadow-md ${isDanger ? 'bg-rose-50 border-rose-100' : 'bg-white border-slate-100'} border group`}>
      <div className="flex justify-between items-center"><div className="flex items-center gap-2"><span className={`text-[9px] font-bold uppercase tracking-[0.1em] px-2 py-0.5 rounded-md ${isDanger ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-500'}`}>{item.publisher}</span>{item.sentiment && <span className={`text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-tighter ${sentimentColor}`}>{item.sentiment}</span>}</div><span className="text-[9px] font-bold text-slate-400 uppercase">{formatTime(item.providerPublishTime)}</span></div>
